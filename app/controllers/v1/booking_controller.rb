class V1::BookingController < ApplicationController
    ...
    def filter 
        @bookings = Booking.joins(:schedule,:payment).search_filter(filter_params).page(filter_params[:page])
    end
    def index
    ...
    def filter_params
        params
            .require(:filter)
            .permit(:location_id, :status, :booking_date_start, :booking_date_end, :page)
    end
end
